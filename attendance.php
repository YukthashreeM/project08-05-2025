...
// Fetch user role and shift assignment for today
$assigned_shift = [];
$role = "";
$today = date('Y-m-d');
...
$stmt = $conn->prepare("SELECT s.shift_name, s.shift_start, s.shift_end, s.id AS shift_id 
...
if (empty($assigned_shift)) {
    ...
    $shift_start = $assigned_shift['shift_start'];
    $shift_end = $assigned_shift['shift_end'];
    $shift_name = $assigned_shift['shift_name'];
...
<div class="container">
    ...
    <a href="<?= htmlspecialchars($dashboard_link) ?>" class="back-btn">Back to Dashboard</a>
</div>
